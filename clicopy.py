...
class AnalyzerGUI(QMainWindow):
    ...
    def _build_left_column(self) -> QWidget:
        ...
        # LIVE TEST RESULTS
        live_panel = QFrame()
        live_panel.setObjectName("Panel")
        live_layout = QVBoxLayout(live_panel)
        live_layout.setContentsMargins(26, 26, 26, 26)
        live_layout.setSpacing(16)
        live_layout.addWidget(self._section_title("Live API Tests", "Results from REST and GraphQL execution"))

        self.live_result_tree = QTreeWidget()
        self.live_result_tree.setColumnCount(4)
        self.live_result_tree.setHeaderLabels(["Test", "Outcome", "Request", "Status"])
        ...
        live_layout.addWidget(self.live_result_tree, 1)
        layout.addWidget(live_panel, 1)
        ...
        functional_layout.addWidget(self._section_title("⚙️ Functional Findings", "Failures discovered from live REST and GraphQL checks"))

        self.functional_findings_tree = QTreeWidget()
        self.functional_findings_tree.setColumnCount(4)
        self.functional_findings_tree.setHeaderLabels(["Issue", "Severity", "Category", "Location"])
    ...
    def _build_right_column(self) -> QWidget:
        ...
        static_panel = QFrame()
        static_panel.setObjectName("Panel")
        static_layout = QVBoxLayout(static_panel)
        static_layout.setContentsMargins(26, 26, 26, 26)
        static_layout.setSpacing(16)
        static_layout.addWidget(self._section_title("📝 Static Finding", "Code review finding details"))

        self.static_detail_title = QLabel("Select a static finding on the left →")
        self.static_detail_title.setObjectName("DetailTitle")
        self.static_detail_title.setWordWrap(True)
        static_layout.addWidget(self.static_detail_title)

        self.static_detail_meta = QLabel("")
        self.static_detail_meta.setObjectName("HintLabel")
        self.static_detail_meta.setWordWrap(True)
        static_layout.addWidget(self.static_detail_meta)

        self.static_detail_body = QTextEdit()
        self.static_detail_body.setReadOnly(True)
        self.static_detail_body.setMinimumHeight(180)
        self.static_detail_body.setPlainText("Select a static finding to inspect code review details here.")
        static_layout.addWidget(self.static_detail_body, 1)
        layout.addWidget(static_panel, 1)
        ...
        live_layout.setContentsMargins(26, 26, 26, 26)
        live_layout.setSpacing(16)
        live_layout.addWidget(self._section_title("🌐 Live Test Result", "REST and GraphQL run details"))

        self.live_detail_title = QLabel("Select a live test finding on the left →")
        self.live_detail_title.setObjectName("DetailTitle")
        self.live_detail_title.setWordWrap(True)
        live_layout.addWidget(self.live_detail_title)

        self.live_detail_meta = QLabel("")
        self.live_detail_meta.setObjectName("HintLabel")
        self.live_detail_meta.setWordWrap(True)
        live_layout.addWidget(self.live_detail_meta)
        ...
        live_layout.addWidget(self.live_detail_body, 1)
        layout.addWidget(live_panel, 1)
        ...
        functional_detail_layout.addWidget(self._section_title("🛠 Functional Finding/Live Test Details", "Why the live test failed and what to do next"))
    ...
    def _populate_live_results(self, functional_summary) -> None:
        ...
        if not functional_summary or not functional_summary.results:
            placeholder = QTreeWidgetItem(["No live tests", "", "", ""])
    ...
    def _populate_functional_findings(self, issues) -> None:
        ...
        if not issues:
            placeholder = QTreeWidgetItem(["No functional findings", "", "", ""])
            ...
            self._set_functional_placeholder(
                ...
                "Live API tests passed or have not been run yet.",
            )
            return
